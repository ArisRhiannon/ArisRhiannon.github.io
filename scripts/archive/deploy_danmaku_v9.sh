...
// Cada línea tiene una "velocidad de fase" propia — algunas van
// más rápido que otras, creando sensación de tiempo distorsionado.
// Las líneas nacen desde un punto de origen en el borde y se
// "desenrollan" hasta completar su órbita (progress 0→1).
// Los glints son micropartículas, NO puntos grandes.
// ════════════════════════════════════════════════════════════════
...
function buildOrbitSeeds() {
  ...
  for (let i = 0; i < N_ORBIT_LINES; i++) {
    ...
    // Punto de nacimiento: ángulo en la órbita donde "empieza"
    const birthAngle = Math.random() * TWO_PI;
    // Velocidad de desenrollado (frames para completar 1 vuelta)
    const unrollSpd  = 0.0008 + Math.random() * 0.003;
    // Progreso actual (inicia en estado parcialmente completo)
...
let lastOrbitT = -999;
const ORBIT_REBUILD = 3; // reconstruir cada 3 frames para animación fluida
...
function drawOrbitLine(seed, tt) {
  ...
  // Color modulado por velocidad de fase (rápidas más brillantes/saturadas)
  ...
  // Punta de la línea: micro-punto brillante (NO punto blanco gigante)
  if (progress > 0.02) {
    ...
    // Punto de cabeza: máximo 2px, solo un pequeño glow
...
// Restaurados más cerca del v5 original: más velocidad angular,
// campo de tiempo más rico, warp más pronunciado.
// Estelas mejoradas: fade más lento (0.038) para que sean visibles.
// ════════════════════════════════════════════════════════════════
const N = 2400;
...
// Historia de posición para trail propio (2 frames)
const bPX    = new Float32Array(N);
const bPY    = new Float32Array(N);

for (let i = 0; i < N; i++) {
  bR[i]     = Math.random() * CMAX * 0.5;
  bAngle[i] = Math.random() * TWO_PI;
  bSpin[i]  = (Math.random()-0.5) * 0.055; // más rápido que v7
...
function updateBullets() {
  for (let i = 0; i < N; i++) {
    ...
    // Campo de dirección más rico: no solo sin sino combinación
    const flow = Math.sin(lt*0.012 + bPhase[i]*0.3) * Math.cos(lt*0.007 - bPhase[i]*0.2);
    const dir = flow > 0 ? 1 : -1;
    bR[i]     += bSpeed[i] * dir;
    // Spin enriquecido: más componentes para movimiento menos determinista
...
function drawBullets() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    // Trail: línea desde posición anterior — da sensación de movimiento real
...
const VORTS = Array.from({length: 3}, (_, i) => ({
  phase: (i/3)*Math.PI*2, r: 100 + i*70, spd: 0.003 + i*0.0015
}));
...
function leaderField(x, y) {
  ...
  vx += Math.cos(a+bg)*0.7; vy += Math.sin(a+bg)*0.7;
  return [vx, vy];
}
...
class Cluster {
  ...
  init() {
    ...
    if (roll < 0.4) {
      ...
      const a = Math.random()*TWO_PI; const r = 20+Math.random()*80;
      this.lx = vx+Math.cos(a)*r; this.ly = vy+Math.sin(a)*r;
    } else if (roll < 0.7) {
  ...
  update() {
    ...
    for(const d of this.dust){
      ...
      if(d.r>26)d.drift=-Math.abs(d.drift); if(d.r<3)d.drift=Math.abs(d.drift);
    }
    const m=120;
...
// ════════════════════════════════════════════════════════════════
// CAPA 3 — Partículas fluidas (SPH-lite, de v8)
// ════════════════════════════════════════════════════════════════
const NF = 3200;
...
for(let i=0;i<NF;i++){
  ...
  const a=Math.random()*TWO_PI; const r=20+Math.random()*180;
  const [vx,vy]=vortCenter(v);
...
const CELL=24; let gridW=0, gridH=0, grid;
function buildGrid(){
  gridW=Math.ceil(canvas.width/CELL)+1; gridH=Math.ceil(canvas.height/CELL)+1;
  grid=new Map();
  for(let i=0;i<NF;i++){
    const cx=(fX[i]/CELL)|0; const cy=(fY[i]/CELL)|0;
...
const REP_R=18,REP_R2=REP_R*REP_R,REP_K=0.35,ATT_K=0.018,DRAG_F=0.92,MAX_V=3.8;
function updateFluid(){
  ...
  for(let i=0;i<NF;i++){
    ...
    for(const v of VORTS){
      ...
      ax+=(dx/r)*ATT_K*0.15; ay+=(dy/r)*ATT_K*0.15;
    }
    const lt=timeField(fX[i],fY[i]);
    const fa=lt*0.04+fPh[i]*0.5;
    ax+=Math.cos(fa)*0.06; ay+=Math.sin(fa)*0.06;
    const cx0=(fX[i]/CELL)|0, cy0=(fY[i]/CELL)|0;
    for(let dcx=-1;dcx<=1;dcx++) for(let dcy=-1;dcy<=1;dcy++){
      const cell=grid.get((cx0+dcx)+(cy0+dcy)*gridW);
      if(!cell)continue;
      for(const j of cell){
        if(j===i)continue;
        const dx=fX[i]-fX[j],dy=fY[i]-fY[j]; const d2=dx*dx+dy*dy;
        if(d2<REP_R2&&d2>0.01){const d=Math.sqrt(d2);const f=REP_K*(REP_R-d)/REP_R;ax+=(dx/d)*f;ay+=(dy/d)*f;}
      }
    }
    ...
    const M=80;
    if(fX[i]<-M)fX[i]=canvas.width+M-1; if(fX[i]>canvas.width+M)fX[i]=-M+1;
    if(fY[i]<-M)fY[i]=canvas.height+M-1; if(fY[i]>canvas.height+M)fY[i]=-M+1;
  }
}
function drawFluid(){
  ctx.globalCompositeOperation="screen";
  for(let i=0;i<NF;i++){
    ...
    ctx.fillStyle=`hsl(${hue|0},${(55+norm*35)|0}%,${(35+norm*30)|0}%)`;
...
// ════════════════════════════════════════════════════════════════
// CAPA 4 — Singularidad central REVISADA
//
// Problemas del v8 resueltos:
//   - Acumulación demasiado lenta → duraciones reducidas
//   - Explosión sosa → energía cinética 3x mayor, onda de choque visual
//   - El ciclo se siente artificial → se añade turbulencia en FLOW y
//     una "onda de presión" visible en EXPLODE que se expande
//
// Ciclo total ~13s (era ~18s):
//   FLOW    220f  (~3.7s) — espiral rápida, singularidad crece
//   CHARGE   45f  (~0.75s) — compresión, pulso tenso
//   EXPLODE  30f  (~0.5s)  — BANG, onda expansiva
//   SCATTER  60f  (~1s)   — fragmentos deceleran
//   RETURN  150f  (~2.5s) — retorno en espiral
// ════════════════════════════════════════════════════════════════
const NS = 800;
...
const SG={FLOW:0,CHARGE:1,EXPLODE:2,SCATTER:3,RETURN:4};
let sgState=SG.FLOW,sgTimer=0,sgRadius=0,sgMaxR=0;
const SG_DUR={[SG.FLOW]:220,[SG.CHARGE]:45,[SG.EXPLODE]:30,[SG.SCATTER]:60,[SG.RETURN]:150};

// Onda de choque de la explosión
let shockR=0, shockAlpha=0;

function initSGParticles(){
  for(let i=0;i<NS;i++){
    const a=(i/NS)*TWO_PI*9+Math.random()*0.5;
    const r=15+Math.random()*280;
...
function updateSingularity(){
  sgTimer++;
  const dur=SG_DUR[sgState];

  if(sgTimer>=dur){
    if(sgState===SG.FLOW){
      sgState=SG.CHARGE; sgTimer=0; sgMaxR=sgRadius;
    } else if(sgState===SG.CHARGE){
      sgState=SG.EXPLODE; sgTimer=0;
      // Calcular vectores de explosión — más energía
      for(let i=0;i<NS;i++){
        const dx=sX[i]-CX, dy=sY[i]-CY;
        const r=Math.sqrt(dx*dx+dy*dy)+0.1;
        // Las cercanas salen más fuerte (más comprimidas)
        const proximity=1+Math.max(0,(80-r)/80)*2.5;
        const spd=(5+Math.random()*14)*proximity;
        sExplVX[i]=(dx/r)*spd+(Math.random()-0.5)*4;
        sExplVY[i]=(dy/r)*spd+(Math.random()-0.5)*4;
      }
      // Iniciar onda de choque
      shockR=sgMaxR; shockAlpha=1.0;
    } else if(sgState===SG.EXPLODE){
      sgState=SG.SCATTER; sgTimer=0;
    } else if(sgState===SG.SCATTER){
      sgState=SG.RETURN; sgTimer=0;
    } else if(sgState===SG.RETURN){
      sgState=SG.FLOW; sgTimer=0; sgRadius=0; initSGParticles();
    }
  }

  // Radius de la singularidad
  const p=sgTimer/dur;
  if(sgState===SG.FLOW){
    sgRadius=p*p*p*60; // cúbico → aceleración al final
  } else if(sgState===SG.CHARGE){
  ...
  // Onda de choque: se expande y desvanece
  if(shockAlpha>0){
    shockR +=14;
    shockAlpha=Math.max(0, shockAlpha-0.025);
  }

  for(let i=0;i<NS;i++){
    if(sgState===SG.FLOW||sgState===SG.CHARGE){
      const dx=CX-sX[i], dy=CY-sY[i];
      const r=Math.sqrt(dx*dx+dy*dy)+0.1;
      const pp=sgState===SG.CHARGE?1.0:p;
      // Pull más fuerte que v8: cuadrático con p
      const pull=0.025+pp*pp*0.09;
      const swirl=0.022+pp*0.018;
      const nx=dx/r,ny=dy/r;
      const tx=-ny,ty=nx;
      sVX[i]+=nx*pull*Math.sqrt(r)*0.12+tx*swirl;
      sVY[i]+=ny*pull*Math.sqrt(r)*0.12+ty*swirl;
      // Turbulencia caótica → movimiento menos determinista
      const lt=timeField(sX[i],sY[i]);
      const turb=0.04+pp*0.06;
      sVX[i]+=Math.cos(lt*0.05+sPh[i]*2.1)*turb;
      sVY[i]+=Math.sin(lt*0.05+sPh[i]*1.7)*turb;
      sVX[i]*=0.90; sVY[i]*=0.90; // drag mayor → más fluido
      sX[i]+=sVX[i]; sY[i]+=sVY[i];
      // Si llega al núcleo, reinyectar desde anillo exterior
      if(r<5){
        const a=Math.random()*TWO_PI; const nr=100+Math.random()*180;
        sX[i]=CX+Math.cos(a)*nr; sY[i]=CY+Math.sin(a)*nr;
        sVX[i]=0; sVY[i]=0;
      }
    } else if(sgState===SG.EXPLODE){
      // Aceleración inicial brutale luego decrece
      const boom=Math.max(0, 1-p*0.7);
      sVX[i]=sExplVX[i]*boom; sVY[i]=sExplVY[i]*boom;
      sX[i]+=sVX[i]; sY[i]+=sVY[i];
    } else if(sgState===SG.SCATTER){
      sVX[i]*=0.86; sVY[i]*=0.86;
      // Leve atracción hacia vórtices en scatter
      for(const v of VORTS){
        const [vx,vy]=vortCenter(v);
        const dx=vx-sX[i],dy=vy-sY[i];
        const r=Math.sqrt(dx*dx+dy*dy)+1;
        sVX[i]+=(dx/r)*0.08; sVY[i]+=(dy/r)*0.08;
      }
      sX[i]+=sVX[i]; sY[i]+=sVY[i];
    } else if(sgState===SG.RETURN){
      // Retorno: espiral inversa hacia anillo propio
      const dx=CX-sX[i],dy=CY-sY[i];
      const r=Math.sqrt(dx*dx+dy*dy)+0.1;
      const targetR=50+(i/NS)*240;
      const radErr=(targetR-r);
      const nx=dx/r,ny=dy/r;
      const tx=-ny,ty=nx;
      sVX[i]+=nx*radErr*0.0015+tx*0.018*p;
      sVY[i]+=ny*radErr*0.0015+ty*0.018*p;
      // Turbulencia decrece con p
      const lt=timeField(sX[i],sY[i]);
...
function drawSingularity(){
  ...
  // Glow de la singularidad
  if(sgRadius>1){
    const pulse=sgState===SG.CHARGE?0.55+0.45*Math.sin(sgTimer*0.9):1.0;
    const hc=(t*0.4)%360;
    ...
    ctx.fillStyle=g1;
    ctx.beginPath(); ctx.arc(CX,CY,sgRadius*3,0,TWO_PI); ctx.fill();

    if(sgState===SG.CHARGE){
      // Anillo de tensión pulsante
  ...
  for(let i=0;i<NS;i++){
    const dx=sX[i]-CX,dy=sY[i]-CY;
    const r=Math.sqrt(dx*dx+dy*dy);
    const norm=Math.min(r/290,1);
    const hue=(sHue[i]+t*0.09+norm*60)%360;
    const spd=Math.sqrt(sVX[i]*sVX[i]+sVY[i]*sVY[i]);
    const spdN=Math.min(spd/12,1); // normalizar contra velocidad explosión máxima
    let alpha=0.12+(1-norm)*0.30+spdN*0.5;
    if(sgState===SG.CHARGE)alpha*=1.6;
...
function loop(){
  ...
  // Fade lento → estelas más largas y fluidas
...
echo "danmaku v9 — líneas orbitales animadas con nacimiento + bullets fluidos + singularidad con onda de choque"
